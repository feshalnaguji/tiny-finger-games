// One-off icon generation: renders public/icons/icon.svg to the PNG sizes the
// PWA manifest needs. Run with: node scripts/generate-icons.mjs
...
// maskable: same art scaled into the 80% safe zone on a solid background
const inner = await sharp(svg).resize(410, 410).png().toBuffer();
await sharp({
  create: { width: 512, height: 512, channels: 4, background: '#4527a0' },
})
